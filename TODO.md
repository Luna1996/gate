...
- [ ] 3.4 环境光近似（v3.5 升级）：常数环境项 → **法线半球 2-3 样本采 sky()**（阴影区被天空色照亮——「阴影不发黑」的关键，与天空/雾共用同一函数）+ 简单体素 AO（接触/缝隙变暗）
...
- [ ] 3.5d **软阴影采样质量调优（v3.5 新增）**：固定 2 锥采样伪影 → 每像素抖动（像素哈希偏移采样序列）+ 样本数评估（2→4）；对照截图 4「干净树影」验收（时域重投影累积归 P9.2，本条目仅空域手段）
- [ ] 3.6 锁定体素全息视觉变体
...
- [ ] 9.7 验收：与 bevy_vox_scene 参考图并排对比超越（暗色实验室场景，RTX 3070 1080p60）；**观感对标锚点补 Douglas Dwyer devlog 截图（v3.2/v3.5 具体化：图1 发光房间 = PT 自发光照明 + 颜色渗透同级；图5 黄金时刻 = god rays + 天空 + 雾综合观感）**
...
| **M2（P3）** | 暗色实验室主题光影实时渲染（**软阴影 + 发光元件直接照明**，v3.2）；**「自然日光」户外主题（程序化天空/距离雾/天空环境光，v3.5）对照 Douglas Dwyer 截图观感验收**；1080p60 @ GTX 1660；万体素编辑不掉帧；理论可上架 | ☐ |
...
| **M3（P9）** | PT GI + 降噪稳定 @ RTX 3070 1080p60，观感超越 bevy_vox_scene 参考图；**god rays 介质散射 + Douglas Dwyer 图1/图5 对照达标（v3.5）** | ☐ |
