...
| **动态体素对象 OBJ** | 独立小 brickmap + 变换（位置/旋转/缩放），**不进世界网格**；渲染 = 世界 DDA + 逐物体局部 DDA（射线变换 + OBB 剔除取最近命中），统一 `trace_scene()` 抽象 | NPC 寄件修复玩法的底座；Teardown/octo 同法先例；两级 DDA 原样复用，物体=小号 TileGrid 全链复用；与「有界工作间」决策不冲突（实体≠流式世界） |
...
流式大世界引擎线（与内容线解耦的引擎能力线）：**P14 全部为引擎能力、零游戏内容耦合**——14.1 tile 驻留管理可在 P3 后任意点插入（机制全是 P2.3/P2.9/P2.10 雏形扩用），14.2 程序化分页 → 14.3 LOD → 14.4 重定基+剔除（依赖 M4 冲刺位），与 OBJ 线并行推进
...
- [ ] 3.5g **Beam 两级渲染 + 阴影降档（对标 #18，1660 降档池首选）**：①**Beam**——低分辨率 pass 完整跑 trace+shade 额外输出 R32F 命中距离 t，全分辨率 pass 以周围 4（斜向布局 6）条低分辨率射线的 **min(t) 为射线起点**（平均空域行程大幅缩短），低分辨率覆盖深度之外从 0 正常步进；**保守性证明**（#18）：低分辨率 pass 限制步进深度，保证「任何大到足以覆盖某全分辨率像素的体素」必被某条低分辨率射线命中 → 输出与非 beam 版**逐像素一致**；**OBJ 物体必须在低分辨率 pass 内参与**（一致性前提）；当前 RenderScale 全分辨率 1:1 无降采样，beam 优于纯半分辨率（着色保持全分辨率且可证明无误差）；②**阴影降档**（beam 思想变体）——阴影是低频信号：半分辨率阴影可见性 + 全分辨率主射线，或软阴影锥采样多样本共享低分辨率可见性（与 3.5d 逐面管线合流：逐面 1 条阴影射线已是天然降档形态）；**降档触发点**：P3.7 验收 1660 不达标启用
- [ ] 3.6 锁定体素全息视觉变体
- [ ] 3.7 性能验收：1080p@60fps（GTX 1660）、每帧 1 万体素编辑不掉帧（基准以 1cm 工作区分辨率计）；**极限压测**：满屏 0.25cm 热点 + 最坏阴影射线路径的帧时间上界断言，VRAM ≤2GB 断言；**外部锚点（#17）**：Douglas GTX 1660 Ti 主射线+阴影射线 7ms/帧（未做 marching 循环优化、无 depth prepass，自评 compute bound 仍有余量）——M2 阴影射线预算可行性的同卡证据，3.5d 软阴影采样有余量
...
- [ ] 4.7 **OBJ-2 平滑移动与交互**：物体 = Bevy Entity（`Transform` 驱动渲染 descriptor，每帧上传，体素网格本身不动 → 平滑无对齐伪影）；抓取 / 拖放 / 放置输入桥（复用 4.x 输入体系与 4.1 拾取——拾取射线同样走 `trace_scene()`）；物体间 / 物体-世界遮挡拾取排序正确
...
- [ ] 10.1 .vox 导入器：MagicaVoxel → palette 映射 → 锁定体素注入（跳过洪泛）；**双目标输出：世界网格注入 + OBJ 物体网格装载**
- [ ] 10.2 工作间雕刻 + 打光（光源为点光源列表，落进「暗色实验室」主题配置）
- [ ] 10.3 元件预制件库：芯片 / 端口 / 显示元件
- [ ] 10.4 背景与工作区隔离：背景 tile 不进洪泛/编辑，一次上传永驻
- [ ] 10.5 **OBJ-3 物体编辑 = 修复玩法**：NPC 寄来的电子产品 = 预制件装载为 OBJ 物体（10.1 双目标 + 10.3 预制件合流）；修复 = 对**物体局部网格**的逐体素编辑（复用 4.2 编辑闭环与脏上传管线，编辑目标切物体槽位）；**动工前先裁决：修复判定语义（图案比对 vs 物体网格进洪泛/模拟），见 UNRESOLVED 表**

## P11 渲染 M4：冲刺线（可无限期推迟）

- [ ] 11.1 ReSTIR 精修直光（M3 降噪稳定后再动）
- [ ] 11.2 玻璃透射（折射射线）；**半透明路线结论（#16 对照）**：gate 全 ray marching **不做 OIT**——OIT 加权平均是光栅化遗产（无序 fragment 才需要排序消除），ray marching 射线天然 front-to-back 有序；水体/玻璃走 **DDA 穿透累积**：命中半透明体素时按不透明度累积 transmittance + 颜色后继续步进（改 `trace_world` 命中分支「不透明度 <1 → 累积+继续」，palette 预留透明通道即可），折射射线后置
- [ ] 11.3 逐体素动画：电流流动（comp_id → 动画参数查表）
- [ ] 11.6 **OBJ-4 烘焙/解烘焙优化（可无限期推迟）**：静止物体烘焙进世界网格（省逐帧多网格遍历 + 纳入 GI/洪泛），拿起时反向解烘焙
- [ ] 11.7 **体素物理避坑清单（对标 #11/#20/#26/#28——gate 自研物理时的施工图与避坑参考）**：
  ...
  - [ ] **破碎/断连（#28/#12）**：硬碰撞分裂物体，对标 Teardown；检测裁剪与物理复用同一套 corner/edge/face/interior 标注；连通分量标记做结构破坏（与 P5 flood-fill 同算法，#12 SVO 同质八分体加速可复用）
...
- [ ] 14.3 **S3 LOD**：可变叶八叉树折叠 = 天然降采样变体（4cm→16cm→64cm）；远处粗叶直出；切换闪烁抑制（抖动溶解 + 距离滞后带）；LOD 变体生成纳入 14.2 分页预算；**#16 补充（渲染距离×3 的服务端方案，与 gate refinement 放大/合并回同构，增量在两点）**：①**LOD 缓存存盘**——生成过的 LOD 落盘，复用 tile 序列化 blob；②**编辑脏区沿 LOD 树向上传播**——用户编辑后标记所有外层 octant「已编辑」，请求高级 LOD 时先读盘缓存、再仅对脏子八分体取高一级 LOD 下采样重组（拉远后编辑依然可见，无需全量重生成）
