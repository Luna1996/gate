...
// LOCAL PATCH (gate): icu_properties 2.3 deprecated `BidiClass::to_icu4c_value` in favor of
// an API tracked at https://github.com/unicode-org/icu4x/issues/6067. Upstream parley 0.9.0
// predates the deprecation, so suppress the warning on the single call site. The replacement
// API (`BidiClass::code_point_class_mask`) is not yet stable as of icu_properties 2.3; revisit
// after icu4x ships the new surface or after parley upgrades the icu4x version it pins.
#[allow(deprecated)]
