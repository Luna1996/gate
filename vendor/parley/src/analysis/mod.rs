...
impl AnalysisDataSources {
    ...
    // LOCAL PATCH (gate): upstream 0.9.0 builds its segmenters with
    // `new_for_non_complex_scripts`, which ships without any complex-script payload.
    // CJK text then degrades to a single unbreakable word/run and icu_segmenter logs
    // `ICU4X data error: No segmentation model for complex script: Chinese/Japanese`
    // on every layout. `new_dictionary` loads the baked compiled_data payloads
    // (SEA dictionaries + cjdict) instead. Cost: a handful of static zerotrie
    // lookups per call; the returned segmenters still borrow `'static` data.
    // Line-side CJK keeps UAX #14 handling per upstream (load_dictionary skips
    // cjdict on purpose, since ID-class breaks between ideographs are correct).
    #[inline(always)]
