...
//!     ⇒ 「这个方向看得见天空吗」× 太阳方向权重 × 太阳色 × 强度 ⇒ 写中间靶 **A**；
...
//! 太阳的**屏幕坐标**在这里算（投影 `view_proj`，见 [`project_sun`]），当它转出画面/转到背后时
//! `朝向因子` 平滑淡出到 0，不会突然消失。
//!
...
pub struct FogUniform {
  ...
  pub misc: Vec4,
  /// x/y 保留（恒 0）、z = 太阳角径（rad）、w = 日晕强度（两项都只喂 `sky_primary`）。
  pub misc2: Vec4,
  /// xy = **太阳的屏幕坐标**（0..1；可能落在画面外 —— 模糊的取样方向就是它）、zw 保留（恒 0）。
  pub misc3: Vec4,
}
...
pub struct FogSettings {
  /// 总开关：关掉 ⇒ 三个 pass 一个都不派发，`dda_main` 的 group(6) 绑 1×1 零纹理（零成本）。
  pub enabled: bool,
  /// **强度**：乘在光柱上。它是最主要的旋钮（"光柱有多亮"），默认 `0.5`。
  pub strength: f32,
  /// **衰减**：径向模糊每步的权重乘它（`< 1`）。越接近 1 ⇒ 光柱拖得越长（几乎不衰减、能到整屏）、
  /// 越小 ⇒ 光柱越贴紧太阳（默认 `0.6` = 短而集中）。
  pub decay: f32,
  /// **集中度**：太阳方向权重的指数 `pow(dot(视线, 太阳方向), 集中度)`。
  /// `1` ⇒ 整个天空都在发光（一层方向性眩光，不像光柱）；`20`（默认）⇒ 太阳周围约十几度内
  /// （`10° ⇒ 0.72`、`20° ⇒ 0.33`）；再大 ⇒ 更尖锐（像激光束）。
  pub focus: f32,
  /// 太阳的**角径**（rad）：天空里那个太阳盘的半径（`sky_primary`）。
  /// 默认 `0` = **只有点状太阳、没有盘**（光柱仍从同一个方向射出来）。
  pub sun_cone: f32,
  /// **日晕强度**：太阳盘外那圈解析拖尾的系数（`sky_primary` 的 halo），默认 `0.5`。
  pub halo: f32,
}
...
impl Default for FogSettings {
  /// 默认值就是菜单「渲染/太阳」面板上那一组：开、**强度 0.5**、**衰减 0.6**、**集中度 20**、
  /// **太阳角径 0°**（只有点状太阳、没有盘）、**日晕 0.5**；网格固定 1/4（见 [`FOG_DIV`]）。
  ///
  /// 这一组的观感是「**紧贴太阳、克制的一束**」：
...
/// 关掉光柱时绑 1×1 的零纹理（uniform 照绑：太阳盘与开关无关）。
...
fn init_fog_gpu(mut commands: bevy::ecs::system::Commands) {
  ...
  bevy::log::info!(
    target: "gate",
    "光柱（godray）：默认 {} / 1÷{} 网格（固定）/ 强度 {} / 衰减 {} / 集中度 {} / 太阳角径 {}° / 日晕 {}；\
