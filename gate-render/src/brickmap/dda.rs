  ...
  fn dda_reference_equivalence_50_rays() {
    ...
    for _ in 0..50 {
      ...
      match (a, brute) {
        ...
        (None, None) => {}
        (a, b) => panic!("hit pattern diff: a={a:?} b={b:?} origin={origin:?} dir={dir_fine:?}"),
      }
    }
  }
  ...
  fn aabb_skip_equivalence_300_rays() {
    ...
    for (eye, target) in &cameras {
      for _ in 0..60 {
        ...
        let full = cpu_reference_dda_ray(
          &bufs, *eye, dir, frustum_len, 2_000_000,
        );
        ...
        let skip = cpu_reference_dda_ray_aabb_skip(
          &bufs, *eye, dir, frustum_len, 2048, aabb_min, aabb_max,
        );
        match (full, skip) {
...
impl Plugin for BrickMapDdaPlugin {
  fn build(&self, app: &mut App) {
    ...
    render_app
      ...
      .add_systems(
        Render,
        prepare_dda_bind_groups.in_set(RenderSystems::PrepareBindGroups),
      )
...
fn dispatch_dda(
  ...
  let (Some(bg0), Some(bg1)) = (bg0.as_ref(), bg1.as_ref()) else {
