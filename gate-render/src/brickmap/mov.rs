...
use super::wire::{
    ...
    SLOT_TAG_EMPTY, SLOT_TAG_LEAF, TILE_BITMAP_WORDS, slot_palette, slot_tag,
    unpack_slot_word,
};
...
pub fn pack_mov_pool(objs: &[MovObject]) -> MovPoolPacked {
    ...
    for obj in objs {
        ...
        out.mov_struct.extend_from_slice(&obj.buffers.b_struct[bitmap]);
        dsc.dir_base = out.mov_struct.len() as u32;
        out.mov_struct.extend_from_slice(&obj.buffers.b_struct[dirs]);
        dsc.node_base = out.mov_struct.len() as u32;
        out.mov_struct.extend_from_slice(&obj.buffers.b_struct[nodes]);
        out.mov_leaves
...
fn extract_mov_scene(
    mut commands: Commands,
    scene: Option<Extract<Res<MovScene>>>,
) {
...
fn desc_transform(dsc: &MovDesc) -> (Vec3, Mat3, f32) {
    (
        ...
        Mat3::from_cols(dsc.rot0.truncate(), dsc.rot1.truncate(), dsc.rot2.truncate()),
        dsc.pos_scale.w,
    )
}
...
fn obj_fine_scan_cell(
    ...
    let p = ro + rd * t_lo;
    let pc = [p.x, p.y, p.z];
    let base = [(cc[0] << 4) as i32, (cc[1] << 4) as i32, (cc[2] << 4) as i32];
    let mut fc = [0i32; 3];
    for i in 0..3 {
...
pub fn cpu_reference_object_ray(
    ...
    let rd = Vec3::new(dir.dot(rot.x_axis), dir.dot(rot.y_axis), dir.dot(rot.z_axis)) / scale;
...
mod tests {
    ...
    fn pack_one(bufs: &BrickMapBuffers, pos: Vec3, rot: Mat3, scale: f32) -> MovPoolPacked {
        pack_mov_pool(&[MovObject { buffers: bufs, pos, rot, scale }])
    }
    ...
    fn pack_layout_bases_sequential_and_content() {
        ...
        let pool = pack_mov_pool(&[
            MovObject { buffers: &a, pos: Vec3::ZERO, rot: Mat3::IDENTITY, scale: 1.0 },
            MovObject { buffers: &b, pos: Vec3::new(600.0, 0.0, 0.0), rot: Mat3::IDENTITY, scale: 2.0 },
        ]);
    ...
    #[test]
    fn rotation_yaw_90() {
        let world = BrickMapBuilder::build_full(&TileGrid::new()).buffers().clone(); // 空世界
    ...
    fn scale_2_doubles_extent() {
        let world = BrickMapBuilder::build_full(&TileGrid::new()).buffers().clone();
    ...
    fn l4_object_brick_slab_offset() {
        ...
        let a = {
            ...
            g.set_voxel(IVec3::ZERO, MAX_LEVEL, 3).expect("杂色细体素写入应生效");
            BrickMapBuilder::build_full(&g).buffers().clone()
        };
        let b = {
            let mut g = TileGrid::new();
            fill_box(&mut g, IVec3::ZERO, IVec3::splat(16), 0, 6);
            g.set_voxel(IVec3::ZERO, MAX_LEVEL, 4).expect("杂色细体素写入应生效");
            BrickMapBuilder::build_full(&g).buffers().clone()
        };
        assert!(a.globals.brick_slabs > 0 && b.globals.brick_slabs > 0);
        // 物体 y 向错开，两条探测射线各只穿一个物体
        let pool = pack_mov_pool(&[
            MovObject { buffers: &a, pos: Vec3::new(500.0, 0.0, 0.0), rot: Mat3::IDENTITY, scale: 1.0 },
            MovObject { buffers: &b, pos: Vec3::new(600.0, 100.0, 0.0), rot: Mat3::IDENTITY, scale: 1.0 },
        ]);
        ...
        let ha = cpu_reference_trace_scene(
            &world, &pool, Vec3::new(2000.0, 8.0, 8.0), -Vec3::X, 8192.0,
        )
        .expect("对象 0 应命中");
        assert_eq!((ha.obj, ha.pal), (0, 5), "命中对象 0 的 brick 体素");
        let hb = cpu_reference_trace_scene(
            &world, &pool, Vec3::new(2000.0, 108.0, 8.0), -Vec3::X, 8192.0,
        )
        .expect("对象 1 应命中");
