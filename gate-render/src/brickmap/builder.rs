...
impl BrickMapBuilder {
    ...
    pub fn build_full(grid: &TileGrid) -> Self {
        ...
        let mut coords: Vec<TileCoord> = grid
            .tile_coords()
            .filter(|&c| {
                index_pos(b.origin, b.dims, c).is_some()
                    && grid.tile(c).is_some_and(|t| t.cell_count() > 0)
            })
            .collect();
    ...
    fn place_tile(&mut self, coord: TileCoord, blob: &TileBlob) {
        ...
        for span in &blob.spans {
            ...
            self.buffers.b_struct[base..base + span.words as usize].copy_from_slice(
                &blob.node[span.start as usize..(span.start + span.words) as usize],
            );
...
mod tests {
    ...
    fn full_build_leaves_no_stale_dirty_marks() {
        ...
        assert!(
            stale.struct_ranges.is_empty()
                && stale.leaves_ranges.is_empty()
    ...
    fn worst_tile_full_depth_stress() {
        ...
        let vram =
            (b.buffers().b_struct.len() + b.buffers().b_leaves.len() + b.buffers().b_palette.len())
                * 4;
        println!(
