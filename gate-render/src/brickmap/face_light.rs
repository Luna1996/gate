...
//! + MOV 版本 + 光照主题变化，由 [`face_light_epoch_tick`] 汇总递增。
//!
...
//! ```text
//!   [0]  status        0 = 空，1 = 占用（key 已写/写中）
//!   [1]  x             命中体素 fine 坐标（世界网格，可负；MOV = 世界 floor）
//!   [2]  y
//!   [3]  z
//!   [4]  obj_key       0 = 世界网格；N+1 = MOV 物体 N
...
//!   [25..27] center    MOV 世界体素中心 xyz（f32 bitcast；世界体素不用，恒 = xyz+0.5）
//! ```
//!
//! CPU 参考实现覆盖世界网格 + identity 变换 MOV（face = 轴向法线量化）；
//! 旋转 MOV 的局部 face 语义由 WGSL 侧保证（F5 集成验收）。
...
use crate::brickmap::mov::{
  MovPoolPacked, OBJ_WORLD, cpu_reference_scene_occluded, cpu_reference_trace_scene,
};
...
/// 新占位槽的 epoch 值（WGSL `FL_EPOCH_INVALID` 镜像）：作废槽内残留
/// （开放寻址槽位随注册竞争顺序漂移，残留 epoch/light 属其他体素），强制重算
...
/// 轴向法线 → face index（非轴向返回 None；MOV 旋转体由 GPU 局部 face 处理）
...
impl Default for FaceLightState {
  fn default() -> Self {
    ...
    Self {
      epoch: 1,
      last_mov_version: u32::MAX, // 首帧必 bump（与 Startup 注入的 version 恒不同）
      last_theme: None,
    }
  }
}
...
/// 体素编辑（dirty 计数 > 0，drain 期间多帧 bump 幂等无害）/ MOV 版本 / 主题变化。
pub fn face_light_epoch_tick(
  scene: Option<bevy::prelude::Res<VoxelScene>>,
  mov: Option<bevy::prelude::Res<crate::brickmap::mov::MovScene>>,
  ...
  if let Some(mov) = mov
    && mov.version as u32 != st.last_mov_version
  {
    st.last_mov_version = mov.version as u32;
    bump = true;
  }
...
///
/// MOV 限制：face 由世界法线量化（identity 旋转时与 GPU 局部 face 语义一致）；
/// 旋转 MOV 的 face 语义由 WGSL 保证（见模块注释）。
...
pub fn cpu_reference_face_light(
  ...
  for (&key, &mask) in registry {
    // 世界体素：epoch 未变 → 跳过（时间复用）；MOV（obj_key != 0）每帧重算。
    ...
    for f in 0..6u32 {
      ...
      let n = face_normal(f);
      // 世界体素中心（identity MOV 与世界同式；旋转 MOV 由 WGSL 用槽内 center）
...
mod tests {
  ...
  use crate::brickmap::{BrickMapBuilder, MovObject};
  ...
  /// 面光照缓存：epoch 不变跳过（值保持），epoch 递增重算；MOV 恒重算
  #[test]
  ...
  /// MOV 物体面参与（identity 旋转：世界 face == 局部 face）
  #[test]
  fn registry_and_composite_with_mov() {
    let world = world_box(512, 3);
    // 小 MOV 物体放在世界盒顶面之上、相机之下（相机 y=640 → MOV 顶 y=584 可见）
    let obj_bufs = world_box(64, 5);
    let pool = crate::brickmap::pack_mov_pool(&[MovObject {
      buffers: &obj_bufs,
    ...
    // MOV 顶面（y=520+64-1=583，相机俯视可见 +Y 面 = face 3）
    let mov_hits: Vec<_> = registry.keys().filter(|(_, obj)| *obj == 1).collect();
    assert!(!mov_hits.is_empty(), "MOV 物体应有可见面注册");
    // MOV 面光照 + 合成
    let lp = build_light_pool(&sun_theme());
    let mut cache: FaceLightCache = std::collections::HashMap::new();
    ...
    for k in &mov_hits {
      let (_, v) = cache[k];
      assert!(v[3].x > 2.0, "MOV 顶面直射 light={:?}", v[3]);
    }
  }
}
