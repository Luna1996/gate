...
//!
//! dda shader 已拆成 WESL 包（`gate-app/assets/shaders/voxel_raytrace/`），由 `wesl-rs` 运行时
//! 编译回单份 WGSL（见 `gate_render::shader::compile_dda_wesl`）；本测试编译同一份包并校验产物，
//! 等价于运行时交给 Bevy 的那份。此处只覆盖 naga 前端与验证器，平台后端差异由实机验收兜底。
...
/// WESL 为单一来源**：Rust 侧启动时解析 `.wesl` 源码（`wesl_consts`）。
///
/// 这里做**端到端**校验：拿编译产物（展平后的 WGSL，即实际交给 GPU 的那份）再解析一遍，与
/// `ddgi_consts()` 逐项对齐 —— 覆盖"解析器和 WESL 编译器看到的不是同一批常量"这类漂移
/// （例如常量写成了派生式、或搬到了解析器扫不到的目录）。
#[test]
...
/// 每层探针数必须**派生**自 AXIS（写死的 256=16² 会与 AXIS 漂移），且 `DDGI_LOD_COUNT` 必须
/// 等于 Rust 的 `DDGI_LODS`（后者要定 `[T; N]` 数组与 uniform 布局的长度，编译期常量无法来自
/// 运行期解析，由 `wesl_consts::load` 断言）。
#[test]
...
/// （`DDGI_ATLAS_LAYERS × 层内轴²`）。位宽不足时下标会在 collect 侧被截断 → `atlas_slot` 折回
/// 低槽位 → 一部分探针的图集被远处探针反复覆写、其余恒空（部分区域全黑），且没有任何
/// 编译/运行时报错。此测试把它挡在 CI。
#[test]
