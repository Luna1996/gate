...
5h. **空间复用借的是"证据量"，不是"位置"**（`GI_SS_REUSE_M_K` + `GI_SS_REUSE_CLAMP_*`）：邻居的 `M`
   ...
   **只限权不够**：复用判据证明的是"同一个**面**"（面键 + 着色法线），**不证明"同一个位置"** ——
   ...
   于是借来的**估计值**再逐通道钳到「本像素当帧自己的估计（`est_self = w_sum / M`，不含历史与借来的量）
   ×`[GI_SS_REUSE_CLAMP_LO, _HI]`」：借来的"位置差异"被切掉，"台阶被抹平"（相邻面差异通常在 2 倍内）
   保留。**第三条**：`prev_ok = false`（重投影失败）时那一帧**不做空间复用** —— 那时没有"本面上帧在
