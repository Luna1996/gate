...
5i. **「拖影 ↔ 静态闪烁」的取舍落在 `GI_DEN_M_MAX`（时域历史长度上限）上**：稳态权重 =
   ...
   （σ 突增 ⇒ `w_var` 掉下来）。
   另有一道**随相机位移收紧**的上限：`w ≤ 1/(1+mo)`，`mo` = 本像素命中点在上帧的落点（导引里现成的
   `prev_px`）与本像素的屏幕距离 ⇒ 记忆窗 ≈ `1+mo` 帧。依据见 5h：接受判据只证明"同一个面"、不证明
   "同一个位置"，其近似误差随位移放大。相机静止 ⇒ `mo = 0` ⇒ 因子为 1 ⇒ **静止画面逐位不变**
   （噪声与累积长度都不动）—— 所以这是"动时压拖、停时压噪"的闸，而不是全程压低累积。
