...
| **GI（屏幕空间 ReSTIR）** | ✅ 生产可用 | 逐像素一个 reservoir（每 GI 像素 `GI_RES_WORDS` 个 word × 2 块 ping-pong），`gi_main` 一次派发完成「新鲜候选（2 条余弦，去遮挡 4 条）→ 时域复用 → 空间复用 → 着色」。复用判据 = **同平面**（面号 + 物体相等 + 沿面法线轴的体素坐标相等；精确面键是其特例）⇒ 薄板/墙缝不漏光。**空间复用零射线**（8 tap 只并上一帧邻居的累计量，同键或同平面），且**只作用在本帧估计、不回写时域历史**。辐亮度 = 单次弹射（命中面按「太阳直射 + 天光×AO」着色后除 π，miss 取天光）⇒ 输入只依赖几何与光照、逐帧确定，第 1 帧即稳态。降噪 = 时域累积（方差驱动历史权重 + **累积矩给出的平滑噪声尺度** + AABB 钳制 + 离群抑制，上限 96 帧）→ 5 轮 atrous（同面键满权重、跨面走几何 × 亮度权重）；回全分辨率用**几何感知上采样**。菜单「渲染/ReSTIR GI」开关 + 分辨率除数（1 / 2）。 |
...
4. **屏幕空间 ReSTIR GI**：逐像素一个 reservoir，每帧「新鲜候选 → 时域复用 → 空间复用 → 着色」，
   再走「时域累积 + 迭代 atrous（5 轮，半径 1/2/4/8/16）」降噪。复用判据是**同平面**
   （面号 + 物体相等 + 沿面法线轴的体素坐标相等；精确面键是它的特例）—— 面在键里 ⇒ 薄板 / 墙缝
   不漏光；同平面 ⇒ 目标函数近似相同 ⇒ 合并累计量的偏差是一阶小量。
   **GI 的输入只依赖几何与光照**（单次弹射，辐亮度不含任何跨帧累积量）⇒ 第 1 帧就是稳态，
   没有「收敛」这件事，也没有相机运动相关的延迟。代价是丢掉二次以上的弹射。
   半分辨率 GI 回全分辨率时用**几何感知上采样**（joint bilateral：只接受与命中面同平面的
...
| `GI_DEN_ATROUS_ITER` | 5 | 迭代 atrous 轮数（步长 1/2/4/8/16，足迹半径 16 个 GI 像素；= NRD/RELAX 的迭代数） |
