...
| 模块 | 状态 | 说明 |
|---|---|---|
| **体素核心（gate-voxel）** | ✅ 生产可用 | Douglas Brick Tree：`HashMap<ChunkCoord, ChunkTree>`，每 chunk 256³ voxel，分裂因子 4³（256 → 64 → 16 → 4 → 1）；层 0-2 = 拓扑节点（u64 子块掩码 + tile 色 + 紧凑 child 表），**层 3 = 定长密集值块**（64 格 inline 值表 + 活跃掩码，形态同 VDB 的 `LeafNode`/GVDB 的 brick）⇒ 一次 4³ 批量写 = 一次 140B 写、零分配、零节点 churn；`Node::Uniform` 表示等值 tile（各层通用）；16 位材质索引（65536 槽，8B/条）；三级查询 `get_voxel` / `get_brick_state` / `fill_brick`（O(深度) 整块写）；`try_merge` 回合并 + `compact()` DFS GC（castle.vox 实测树节点 77 万，值块化之前 ≈1200 万） |
...
| `GI_SS_REUSE_TAPS` | 8 | 空间复用 tap 数（3×3 环；同一个面 ⇒ **只并累计量，零射线**）。**重投影失败（`prev_ok = false`）的那一帧不做**：那时没有"本面上帧在屏幕哪儿"这个锚，退回本像素坐标读邻居等于借一个无关的世界点（相机前进/后退时大批像素落在这条路上，见 5h） |
| `GI_SS_REUSE_CLAMP_LO` / `_HI` | 0.5 / 2.0 | 空间复用借来的**估计值**逐通道钳到「本像素当帧自己的估计 × `[LO, HI]`」。**上限只压权重不够**：判据证明的是"同一个**面**"、不证明"同一个**位置**"（面键允许沿平面任意平移）⇒ 相机动时邻居位置上的值（及其长历史）会被搬进来，在高对比处（背光面暗部边界）成拖影（见 5h） |
...
| `GI_DEN_LUMA_FLOOR_REL` | 0.05 | φ 的**相对下限**（× 邻域平均亮度）—— 只是最后一道地板：φ 的主尺度是**实测**噪声量（同面键 σ / 累积矩；同面键邻域只剩自己时取共面邻域的实测离散度）。固定百分比的地板当主尺度会按场景明暗分段失效：GI 的**相对**波动随变暗而变大（暗处逃逸到天空的射线远亮于任何被照到的面）⇒ 暗部的 `|Δluma|` 远超地板 ⇒ atrous 的 range 权重塌成 0、五轮空转 ⇒ raw 噪声直通 |
...
5g. **历史搜索只改「去哪儿找」，不改判据**：`GI_HIST_SEARCH_TAPS` 的每个候选一律仍走
   ...
   明写在案的路。
5h. **空间复用借的是"证据量"，不是"位置"**（`GI_SS_REUSE_M_K` + `GI_SS_REUSE_CLAMP_*`）：邻居的 `M`
   上限与我们**相同**（`GI_SS_M_CAP_K` × 本帧候选数），而 `GI_SS_REUSE_TAPS` 个 tap 合计可达它的数倍
   ⇒ 必须按 `w_sum` / `M` **同比例**限权（`GI_SS_REUSE_M_K`），否则本帧那几条新鲜样本被完全淹没，
   画面被"**上一帧 + 屏幕偏移一格**"的值支配（症状：相机一动就拖影、暗块像钉在屏幕上）。
   **只限权不够**：复用判据证明的是"同一个**面**"（面键 + 着色法线），**不证明"同一个位置"** ——
   面键的切向坐标不进判据，于是相机靠近 / 远离时（`prev_pc` 附近那 8 个 tap 对应的世界位置随距离
   缩放滑动）邻居**位置上的值**及其长历史会被搬进来，在 GI 高对比处（背光面的暗部边界）成拖影；
   静止时 tap 对称 ⇒ 只是一次无方向性的等权模糊，**所以只有相机动才看得出来**。
   于是借来的**估计值**再逐通道钳到「本像素当帧自己的估计（`est_self = w_sum / M`，含时域合并、
   不含借来的量）×`[GI_SS_REUSE_CLAMP_LO, _HI]`」：借来的"位置差异"被切掉，"台阶被抹平"（相邻面差异
   通常在 2 倍内）保留。**④ 时域合并因此排在 ③ 之前** —— 锚不含时域历史时它自己是那几条新鲜候选的
   噪声，拿噪声锚去钳邻居的长历史会把锚的偏倚带进画面（静态可见的轻微噪声）。**第三条**：`prev_ok = false`（重投影失败）时那一帧**不做空间复用** —— 那时没有"本面上帧在
   屏幕哪儿"这个锚，退回本像素坐标读邻居等于借一个无关的世界点；而相机前进/后退恰恰大批像素落在这条
   路上（命中点被推到视锥外 / 落进相机背后、新入屏的轮廓像素）。
   另注：`hist_ok = false` 会**同时**触发「去遮挡」候选翻倍 ⇒ 任何"把历史判得更严"的改动（哪怕是
   临时诊断）都会成倍放大射线开销，别把它当免费手段（上面三条都不动 `hist_ok`，零射线代价）。
5i. **「拖影 ↔ 静态闪烁」的取舍落在 `GI_DEN_M_MAX`（时域历史长度上限）上**：稳态权重 =
   ...
   （σ 突增 ⇒ `w_var` 掉下来）。
   另有一道**随相机位移收紧**的上限 `w ≤ 1/(1+mo)`（`mo` = 本像素命中点在上帧的落点与本像素的
   屏幕距离），但它**只作用于"靠搜索挪开落点"才接上的历史**（判据只要求同平面 + 法线相似，切向坐标
   不进判据的后果）。正中命中（落点本身）的历史是同一个世界点的同一个积分 ⇒ 与位移无关，不吃惩罚。
   这条区分是必须的：被小发光体照亮的暗面**全靠累积**才干净（每帧只有 4 条候选、命中发光体的概率
   极低，逐帧原始估计是重尾的），一刀切按位移压权重会把它的累积抽掉 —— 症状：静止干净、一动就
   回到 raw 噪声、停下后要几秒才攒回来。相机静止 ⇒ `mo = 0` ⇒ 因子为 1 ⇒ **静止画面逐位不变**。
5j. **累加量的前提被破坏 ⇒ 该帧 reservoir 不复用任何历史**（门 = `gi_u.flags.z`，见 `gi/screen.wesl`
   文件头 ⑥）。两个触发源都在 Rust 侧判定（`prepare_gi`）：
   · **几何**：世界的体素占据变了（`world_rev`，覆盖一切增量体素编辑 / palette 变化 / 全量上传）；
   · **光照阶跃**：主光 / 天光一帧内相对变化 > `LIGHT_STEP_MAX`（0.5 ≈ 50% 量级或 60° 方向），
     用 `light_jump` 从 epoch key 上算（`gi.rs`）。为什么要这一条：二次顶点缓存被 epoch 作废 ⇒
     本帧新鲜候选算的已经是新光照，而 reservoir 里是旧光照攒的 `w_sum` / `M` ⇒ 两侧时间基准劈开，
     靠记忆窗淡出会整屏滞后、各像素不同步，相机一动还会借到旧值成大鬼影（症状：切时刻后场景
     十几秒不暗、一动相机就"一部分像素突然明白自己要暗下来"）。逐帧小变化（自动流逝 0.5 游戏
     小时/秒 ≈ 0.125°/帧、常规拖动滑杆步长）**不触发** —— 那种变化每个窗口帧只有一小步。
   为什么要这么粗：reservoir 存的是 `w_sum` / `M` 这条**累加量**，而验证射线只重发**被选中的那一个
   代表样本**的光路 —— 代表样本没变**不代表**累加量里别的样本还有效。症状：擦掉一块自发光体素 ⇒
   接收面上"打到它"的候选全部失效，但代表样本（例如一条逃向天空的方向）仍然成立 ⇒ 整条累加量照收
   ⇒ 那束光只能靠记忆窗淡出（**相机不动时看不出任何即时变化**）。这条粗门也正是 `GI_RES_WORDS`
   能从 14 缩到 8 的原因（逐样本校验、`gi_ray_secondary_key`、代表样本字段全部删除）。
