...
use gate_render::{
  BrickMapBuffers, BrickMapBuilder, DdaCameraConfig, DdaImages, GradientImages, GradientUniforms,
  MovObject, MovScene, OrbitCamera, UploadBudget, VIEW_SIZE, VoxelScene, create_dda_image,
  create_gradient_image, pack_mov_pool,
};
use gate_ui::{
...
fn build_chip_prefab() -> BrickMapBuffers {
  ...
  for s in 0..4 {
    fill_box(&mut g, IVec3::new(8 + s * 32, 16, 0), IVec3::new(16, 8, 8), 0, 3);
    fill_box(&mut g, IVec3::new(8 + s * 32, 16, 120), IVec3::new(16, 8, 8), 0, 3);
    fill_box(&mut g, IVec3::new(0, 16, 8 + s * 32), IVec3::new(8, 8, 16), 0, 3);
    fill_box(&mut g, IVec3::new(120, 16, 8 + s * 32), IVec3::new(8, 8, 16), 0, 3);
  }
  // L4 走线（1-fine 细线，PCB 上表面 y=32）
  for i in 0..7 {
    fill_box(&mut g, IVec3::new(16 + i * 16, 32, 16), IVec3::new(1, 1, 96), 4, 2);
    fill_box(&mut g, IVec3::new(16, 32, 16 + i * 16), IVec3::new(96, 1, 1), 4, 2);
  }
