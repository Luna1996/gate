...
**MT8-2 同日按用户复查意见重做**（只对反光/高光按面，见 `§8` 最后一节"反光/高光逐体素"）。

**为什么这四件事是一个改动**：采样点落到体素格上之后
① 反射/高光/透明**自动**逐体素恒定（同一份 `MaterialParams`）—— ⚠️ 这句在 2026-09-21 曾被实测推翻：
`MaterialParams` 只管**材质**，而反光/高光还吃**视向量**（当时仍逐像素）⇒ 真正逐体素要等 `face_view_dir`
（`§8` 最后一节）才成立；
...
### 2026-09-21 · 反光/高光逐体素（MT8-2 的"只按面"版回来）

用户复查截图："**当前渲染链路中还有不是逐体素着色的部分。重点调查反光、高光**"（截图 = 地板上的一片平滑渐变）。

#### 调查结论（逐体素 / 逐像素的清单）

**已经逐体素**：albedo/roughness/metallic（MT8-1 的 `material_sample_pos`）、着色法线（`voxel_normal`，纯函数）、漫反射 `N·L`、AO（`light_field_ao` 采样点 = 体素中心）、抖动种子（MT8-6）、emissive。

**仍逐像素：只有视向量 `v`**（`view_dir_to`，主 pass 传 `best.point`）—— 而本工程**所有**读 `v` 的量恰好都是反光/高光（`brdf_reflected` 里 `v` 只出现在太阳 GGX 那一支；漫反射 `kD·albedo·N·L` 与环境/GI `(kD·albedo+F0)·(amb+gi)` 都不读它）⇒ 表现就是"材质色逐体素成块（MT8-1），光照却跨体素连续"：

| 症状 | 机制 |
|---|---|
| 截图那片平滑光晕 | 反射权重 `fresnel_schlick(f0, v·n)` 逐像素 ⇒ 掠射方向一片单调变亮 |
| 跨体素的连续亮带 | GGX 高光 `h = normalize(sun_dir + v)`：平面上所有体素同一个 `n` ⇒ 它是屏幕位置的连续函数 |
| 反射内容连续（MT8-6 之后仍如此） | 种子虽按面，但基准方向 `mirror = reflect(-v, n)` 逐像素 ⇒ 同面内仍各采一个方向 |
| GI 里烘进去的高光更平滑 | `gi/ray.wesl` 二次顶点用同一套 `brdf_reflected` + 逐像素 `v`，再经降噪 + 联合上采样 |

#### 改动（`common.wesl` / `main.wesl` / `gi/ray.wesl` / `world.wesl`）

新增唯一契约 **`common.wesl::face_view_dir(射线起点, 命中体素中心) = view_dir_to(...)`**（公式仍只有 `view_dir_to` 一份，硬约束 3 不分叉），**五个着色点全部改走它**：主 pass（GGX 高光 / `mirror` / 反射权重）、MT4 反射命中点（该处 `sun_c=0` ⇒ **无行为变化**，只为口径统一）、`trace_glass` 界面 Fresnel 与反射方向、玻璃反射命中点、GI 二次顶点。
**光路几何量未动**：`ci`、折射/全反射、`t` 全用精确方向（那些不是着色量）。

#### 已知代价（本档的预期，不是 bug）

- 掠射面**整面**变亮（= 用户当初看到的"白边"）—— 逐像素时它只是最外一列像素的渐变；
- 反射变成**块状镜面**。当年"光滑度拉满也看不到镜面"的实测发生在 **MT8-3 陈旧缓存**（刷新 K=16 + 命中复用）还在时，缓存已删除 ⇒ 这次应能看到"按体素格采样的镜面"。

#### 验证与待人工

- **已验证（机械）**：WESL 包编译通过（`gate-render::shader::compile_dda_wesl()` = 87764 字节，`cargo test` 临时用例跑完即删）。**未做**任何视觉验收。
- **待人工 ①**：截图位置复查 —— 那片跨体素的平滑渐变应变成**按格成块**（与几何台阶对齐）。
- **待人工 ②**（决定本次是否成立）：`F3 → 游戏/编辑` + PBR 资产：`金属度 = 100 / 光滑度 = 100` 落一个方块 ⇒ **应能看到块状镜面里的环境**。若仍完全看不到镜面，说明 MT8-2 当初"逐面恒定 ⇒ 镜面不可能出现"的结论才是真的 ⇒ 必须回来在"逐体素"与"镜面"之间做取舍（不要把它当 bug 直接改回去）。
- **待人工 ③**：轮廓上的"白边/整面变亮"是否可接受（若不可接受，最小收敛手段是给反射权重设上限或按面渐变抑制，而不是退回逐像素）。
- **A/B 手法**：`PBR_REFLECTION_ENABLED = 0u` 关掉环境反射（排除第 1 项）；菜单「渲染/RESTIR GI/太阳反弹」= 0 关掉 GI 里的二次顶点高光（排除第 4 项）；`PBR_DEBUG_ASSET = 10u`（metal_plate）把反光/高光放大到肉眼可见。
