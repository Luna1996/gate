...
- [~] M4-3 级联滚动 + reuse bounds（CPU 数学 done `4f56b67`：`cascade_scroll_origin`（相机→级联原点，cell 对齐+居中+亚 cell 不动点）+ `scroll_reuse_bounds`（半开区间分带），负坐标 floor 单测锁定；**GPU 多级联接线未做**——设计已定：①base 世界级（现状，静态）+ 4 级滚动级联（`bake_cascade_grid` 16³/级，滚动时 CPU 重烘）；②纹理层分配 = base 层 [0, base_layers) + 级联 c 层 [base_layers + c×16, +16)；③滚动继承 = **CPU meta 重映射**（级联 meta 仅 4096 word，CPU 读旧 meta 按 shift 搬运 age/offset 重传，规避 GPU 移位拷贝的坐标耦合）+ irr/depth 用 GPU 分层 shifted copy（z 轴逐层 copy_texture、xy 轴 origin 偏移裁剪）；④pass 编排 = 每级联一组 clear→active→seal→cast→update（uniform 换 grid_origin/dims/finer；base 的 finer = LOD1，级联 c 的 finer = c-1）⑤WGSL ddgi_active 已读 uniform finer/reuse 字段（M4-1 就绪）无需改
- [ ] M4-4 编辑响应重接
- [ ] M5-1 旧代码清除
- [ ] M5-2 全量测试绿
- [ ] M5-3 bench A/B（不劣化）
