...
- [~] M4-3 级联滚动 + reuse bounds（CPU 侧 done：`4f56b67` 滚动数学 + `101027d` **CascadeManager**（4 级 origin/grids/metas/reuse 快照；scroll() = 重烘 + meta 重映射搬运 age，单测验证重叠区保留/新列归零）。**GPU 多级联接线未做**——设计已定：①base 世界级（现状静态）+ 4 级滚动级联（CascadeManager 驱动）；②纹理层分配 = base 层 [0, base_layers) + 级联 c 层 [base_layers + c×16, +16)，**级联探针 id = base_layers×256 + c×4096 + slot**（id 空间偏移使 WGSL coord 函数零改动；base_count 非整层由取整规避）；③滚动继承 = CascadeManager.metas CPU 重传（4096 word/级）+ irr/depth GPU 分层 shifted copy（z 逐层 + xy origin 裁剪，写 next 后 swap）；④pass 编排 = 每级联一组 clear→active→seal→cast→update + 独立 BG4（positions 共享缓冲 id 索引、cell_index/worklist/samples/dispatch/indirect 每级联独立，级联 samples ≤4096×2 vec4）；⑤ddgi_sample 距离选级 + 过渡带混合（M3-4 尾款：base 优先，base 无有效探针 fallback 级联粗探针）⑥WGSL ddgi_active 已读 finer/reuse 字段无需改
- [ ] M4-4 编辑响应重接
- [ ] M5-1 旧代码清除
- [ ] M5-2 全量测试绿
- [ ] M5-3 bench A/B（不劣化）
