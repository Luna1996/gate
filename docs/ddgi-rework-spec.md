...
- [x] M3-1 ddgi_active（CPU 镜像 + 10 单测 + WGSL 镜像 done，commit `2551233`/`186e223`）
...
- [x] M4-2 三 pass 编排 + indirect 一致性（commit `ac8fc48`；目验反馈修复 `e89422c`：dda_main 补 R3-18 太阳硬阴影射线 + **逐体素锚定着色**（一体素一色）+ CPU oracle 日志。**wgpu 验证坑**：copy src==dst 同 handle 非法（烘焙建 6 张独立纹理）/ 纹理 usage 缺 COPY_SRC / 同 dispatch scope 内 STORAGE 与 INDIRECT 互斥（拆双 buffer + encoder copy 桥接）/ **dispatch 超限静默跳过 `96cd8f4`**：cast/update indirect x=活跃数（demo 21 万）超 `max_compute_workgroups_per_dimension=65535` → WebGPU **静默跳过无报错** → cast/update 从首帧从未执行。修复 = seal pass（全量 count → `min(count, DDGI_PROBE_BUDGET=4096)` 写 dispatch[1]）+ 桥接取 dispatch[1..4]，workgroups ≤ 4096，count 读 dispatch[1]——4096/帧 = Douglas 固定射线预算本义，未覆盖探针跨帧轮换 ~1.4s。同修 halo 域内格漏写（skip 规则跳过 d=0 owner 格 → intersection=0 → 30 万探针全活跃）→ 循环前先写自己位。staging 三阶段回读 + oracle + 绿点标记留存（M5-1 评估清理）。回读实证：indirect0=4096、采样层非零 texel=2（绿点+真实收敛各 1））
- [x] M4-3 级联滚动 + reuse bounds（CPU 侧 done：`4f56b67` 滚动数学 + `101027d` **CascadeManager**（4 级 origin/grids/metas/reuse 快照；scroll() = 重烘 + meta 重映射搬运 age，单测验证重叠区保留/新列归零）。**GPU 多级联接线 done（2026-09-07）**：①base 世界级 + 4 级滚动级联（CascadeManager 驱动，pending_shift 记录本帧位移）；②纹理层分配 = base 层 [0, base_layers) + 级联 c 层 [base_layers + c×16, +16)，**级联探针 id = id_base + c×4096 + internal_id**（id_base = base_layers×256；cell_index 上传时经 `cascade_ci_global` 平移为全局 id，positions 按 internal id 落位，meta = 内部 id 布局原样）；③滚动继承 = meta CPU 重传（写 prev，meta copy 扩到 base+64 层传播）+ irr/depth **shifted copy**（dispatch 编码器：无滚动 shift=0 → 整域 16 层单次 copy；滚动帧 → 逐层 xy 重叠矩形搬运）+ **新列复位**（prepare 队列直写 prev/next 两份：irr=0 / depth=tmax，与 shifted copy 重叠区不相交）；④pass 编排 = base 链后每级联一组 clear→active(4³ wg)→seal→桥接 copy→cast→update（indirect），BG4 v3 **15 binding**：0/2/9/11/12 本级 uniform/ci/dispatch/samples/worklist，1/3-8/10/13/14 共享；⑤ddgi_sample 多级联 done（BG4 加 binding 13 = **DdgiDomains 560B 全域 uniform**（base+4 级）+ binding 14 = all_ci（base ci ++ 级联 ci）；8-cell 三线性核心按域参数化 `ddgi_sample_dom`（base step16=1 退化原版），base 优先 → 级联由细到粗首个有效级 fallback）；⑥base uniform finer = LOD1 域、级联 c finer = c-1 域（D4 归属剔除接通）。单测 +4（domains wire 560B/axis_shift_range/scroll_reset_rects/ci_global）；114 测试全绿。**已知取舍**：CPU meta age 快照跨滚动陈旧（age 仅门控 can_skip，回归无正确性影响）。**目验修订（2026-09-07，帧率极低+卡顿）**：⑤改为**确定性单域选样**（`ddgi_dom_contains` 纯 ALU 选域，每像素恰一次 8-cell 采样——逐域 fallback 在 base 域外走满 5 次 8-cell = 5×纹理 load 打穿帧预算；优先级 casc0→base→casc1/2/3）；滚动改**每帧至多滚一级**摊销（最细优先，粗级顺延一帧）；移除 ddgi_update 绿点调试写入（多级联后每帧污染 5 个探针 irradiance）。**二次目验修订（3ms GPU 但巨卡 + 光斑不消退）**：⑦**级联 id 改 slot 身份**（id = 线性 cell 下标，positions 按 slot 预分配；此前扫描序 internal id 使 texel 布局 ≠ cell 网格，GPU shifted copy 的「矩形平移 = cell 平移」假设不成立 → 每次滚动把 irr/depth 搅成随机斑块永不收敛 = 光斑污染根因）；⑧滚动改**增量烘焙** `bake_cascade_grid_shifted`（重叠 cell 零下钻搬运，仅滚入列带现场烘焙 ~16×，`probe_leaf_sized` 对 256³ Mixed cell 最坏 8⁴ 下钻 × 全量 16³ = 滚动帧秒级 CPU 停顿 = 卡顿根因，GPU 计时 3ms 但帧间 CPU 风暴）；⑨滚动帧 **next→prev 镜像回拷**重叠矩形（update 的 EMA 基准读 prev，单向搬运后 prev 仍是旧格错位数据）；⑩回读扩容：每级联 dispatch 全量/钳制/indirect 计数入日志（c0:full/seal/ind …）；⑪**每探针射线数下限 `DDGI_PROBE_RAYS_MIN=16`**（对照 douglas-final.md §5.2「每帧总射线数固定摊给活跃探针」——我们 4096 预算摊给 4096+ 活跃级联探针 = 1 ray/探针纯噪声，EMA 永不收敛；16 ray 下限 × 5 pass ≈ 0.9ms GPU）；⑫**样本缓冲越界发散修复**：ddgi_cast/update 的样本下标原用「旋转后全量 slot」（可达 21 万）×rays → 冲出缓冲（越界读恒 0 → EMA 暗化发散、写钳制互踩 → 亮斑）=「放越久斑越重」根因；改为**本帧处理序号 wid.x**（< 钳制值 4096）×rays，旋转窗口仅用于 worklist 查找（CPU 镜像契约：worklist 紧凑下标）。画质与 Douglas 截图的残余差距 = 稳态收敛时间（他静态场景跑了几分钟）+ R6 纹理未做（纯色 albedo）
- [ ] M4-4 编辑响应重接
- [ ] M5-1 旧代码清除
- [ ] M5-2 全量测试绿
- [ ] M5-3 bench A/B（不劣化）
