...
| **M8** | **多级远场（clipmap 式）〔已落地，见 §9；验收 = 视距 ≥ 5 km @ 1080p，待实跑〕** | 近场（L0，现状 ±164 m）之外按 **粒度 ×4 / 覆盖 ×4** 加 3 级：每级 = 一个**带 `scale` 的额外 volume** + 自己的 64³ 窗口 + "每格一次采样"的远场产出；装载**只由射线请求驱动**（远场不铺满）；DDA 按"上一级覆盖半径"给每级**分壳裁剪**（`t_min`） | 梯级（1080p，1 px = d/935）：`g 2cm → ±164m`、`g 1.28m → ±655m`、`g 5.12m → ±2.6km`、`g 20.5m → ±10.5km` ⇒ **L3 覆盖 5 km**；每级接缝外沿 1.8 px / 内沿 7 px；远场每 chunk 实测 4–6 KB（grain 16，见 §9 M8） | — | `scene`/`infinite_cubes` 多级化 + `snapshot` 的窗口 AABB + shader 分壳；中高 |
...
| 视距 | 1080p：全分辨率半径 ≥ 4 chunk（20 m）；三级 proxy 在本仓 **2 cm/体素**下约 75 m / 300 m / 1.2 km；**主世界窗口的水平硬上限 = ±164 m**（见 §3.4 注 2）。**M8 起**：该上限由三级远场 volume 接力突破 —— L1 ±655 m / L2 ±2.6 km / **L3 ±10.5 km**（格 1.28 m / 5.12 m / 20.5 m）⇒ **5 km 落在 L3 内** | §3.4 的算术（已按 2 cm 修正）+ §9 M8 的实测；运行时以启动的 `FAR L{1,2,3} …` 三行、`b_struct` 大小与 `STREAM`/`RESID`/`REQ` 日志核对 |
...
### M8：多级远场（clipmap 式）—— 已落地（**待实跑验收**）

**梯级**（`infinite_cubes::{FAR_SCALES, FAR_GRAIN, far_coverage_voxels}`，单测 `far_ladder_covers_five_km`）：

| 级 | `scale`（级体素 = 几个主世界体素） | chunk = `256·scale` 体素 | 覆盖半径 `32·256·scale` | 格 `FAR_GRAIN·scale` |
|---|---|---|---|---|
| L1 | 4 | 20.5 m | 32768 体素 = **655 m** | 64 体素 = 1.28 m |
| L2 | 16 | 81.9 m | 131072 = **2.6 km** | 5.12 m |
| L3 | 64 | 327 m | 524288 = **10.5 km** | 20.5 m |

⇒ **5 km 落在 L3 内**（§10.4 的验收目标）。`CAM_FAR` 随之从 65536 提到 **1048576 体素**（≈ 21 km，含
L3 的对角）—— 主射线的 `t_cap` 由它推出（`main.wesl`），不提就看不到远场。

**三处机制**（都复用既有设施，这是 M8 便宜的原因）：

- **每级 = 一个带 `scale` 的额外 volume**（`Volumes::add_far_level`，`pos = 0 / rot = I` ⇒ 格点锚在
  世界坐标上，窗口滑动不改相位 ⇒ 远处不闪）；窗口 64³ chunk、逐帧钉在相机中心，坐标换成该级的
  chunk 空间（`相机世界坐标 / scale / 256`）⇒ `builder::set_window` 的**平移索引区一字不改**。
- **请求通道带卷号**：`trace_grid` 的请求字加 2 bit 卷号（`GRID_VOLUMES = 4`，与 Rust
  `brickmap::consts::VOLUMES` 相等，`wesl_consts` 会核对）；用途戳表按卷分段（远场级的 chunk 坐标与
  主世界**数值上会撞**，不分段互相顶掉）。`plan_generation_far` = 只收本卷 + 窗口内 + 还没挂载的请求，
  票数降序；**远场不做半径补块**（铺满一级 = 262144 chunk ≈ 17 GB）。
- **分壳裁剪**（`world.wesl::trace_scene` + `trace.wesl::aabb_exit`）：远场级只从**上一级世界 AABB 的
  出口**起参与求交。用 AABB 出口而不是"窗口半宽"这个常数：窗口是相机居中的盒 ⇒ 同一级沿不同方向的
  覆盖在 `R..√3R` 之间，用常数会在对角方向让两级各画一份。
- **常驻**：CPU 侧按"容量 + 用途戳 LRU"裁（与主世界同一套代码，容量见下面偏差 ②）；GPU 侧
  `plan_residency` ⑥ 段只做"CPU 有 ⇒ GPU 有 / CPU 没了 ⇒ 归还"（远场本身已是粗档，没有"再塌一层"），
  每帧上限 `FAR_INSTALL_PER_FRAME = 24`。

**三处与 §10.4 的偏差**（都有实测/算术依据，不是实现走样）：

① **`FAR_GRAIN = 16`（§10.4 写的是 4）**。实测（单测 `far_detail_is_much_smaller_than_full`，
`infinite_cubes` 同一 chunk）：

| grain | 格（L1，世界体素） | 树（wire） | 本级内沿的接缝像素 |
|---|---|---|---|
| 4 | 16 | **418 KB** —— 比全分辨率（288 KB）还大 | 2 px |
| 16 | 64 | **4–6 KB**（50–60× 便宜） | 7 px |

`grain = 4` 时格（16 体素）比白柱截面（24）还细 ⇒ 远场把柱体的**截面**也画出来，采样结果是"孤立
的一格格"、父层无从合并 ⇒ 树比近场还大。**§10.4 的"处处 ≤ 1 px"是那条判据的理论值，买不到可承受的
内存**；本轮取 16，明确接受内沿 7 px 的块（外沿恒 1.8 px）。

② **远场池容量单独换算**（`gate_render::pool_capacity_chunks_far`，32 KB/块）：主世界那 1 MiB/块 是
"可编辑的全分辨率树"的口径；用同一个换算会把远场卡在 2048 块，而**视锥内一级 ≈ 1.5 万块**
（壳层体积/块体积与级数无关：`≈ 4π/3·(1−k⁻³)·32768·f`，`f` = 视锥立体角占比 ≈ 0.117）⇒ 会在大半个
锥里留**成片空洞**。取 `budget_bytes/4` ⇒ 默认 2 GB 时每级 ≈ 16384 块。

③ **请求闸门 ① 对远场跳过，且回读按卷配额**。`REQ_LEVEL_KEEP`（只报 `fp < 16` 的）问的是"这个距离
值不值得加载细节"，而远场级的 chunk 尺度本身就是按那个距离选的 ⇒ 远场一律报 0 档；不跳则 L2 起
`req_level ≥ 2` 恒被滤掉、**远场永远空着**。回读侧 `REQ_FEED_MAX` 改成**按卷各一份配额**：主世界的
票数天然高得多，全局截断会把远场那点票挤没。

④ **远场 volume 的树区必须预留固定长度**（`FAR_TREE_RESERVE_WORDS` = 16 MB/级）。远场排在主世界之前，
它的 `b_struct` 一变长就会让主世界的 `tree_base` 漂移 ⇒ `bases_shifted` 降级**全量重传**
（实测 577 MB / 100–200 ms/帧 ⇒ 帧率掉到个位数）。这条与 §8 的"物体变长用增长余量摊薄"是同一条，
只是远场级的增长是**每装一块都发生**，不预留就必然踩。详见下面"第二轮实跑"。

**一处世界特性（不是 bug，决定怎么看画面）**：`infinite_cubes` 是**三维杆网格**（周期 148 体素、
杆截面 24）—— 水平射线的平均自由程 ≈ 18–24 m（`exp(−d/λ)`），只有**几乎与轴平行、且落在杆之间
"走廊"里**的方向（≈ 1% 的屏幕立体角）能穿过近场看到远处。所以：远场也只在这些方向上被请求、被
绘制 ⇒ **视觉验收要看走廊方向**（顺着杆的走向看），不是随便一瞥。

**首轮实跑（用户机，`logs/latest.log`，720p、vsync 60）**——机制**端到端跑通**，另有两处代价：

- 启动三行 `FAR L{1,2,3} …` ✓；`REQ[...]` 出现 `v1/v2/v3` ✓ ⇒ 请求 → 产出 → 挂载 → 绘制整链成立。
  稳态 `REQ` = 每窗 200–900 条 / 4–40 个去重 chunk、`用途戳` 400–700 chunk ⇒ 装载按视线推进。
- ⚠️ **首窗 1200 万条请求（超容丢失 1098 万）**：相机刚进世界、三级远场全空时，远场每一步
  `entry == 0` 都在发请求，而请求追加是**单地址原子加**（`lod_req[0]`）⇒ 正是 §9 M4 记过的
  "帧率崩到个位数"同一形态（那次是每窗 4.8 亿条）。**修**：远场用独立的、更稀的采样分母
  （`trace.wesl::REQ_SAMPLE_FAR = 64`，主世界仍是 16）⇒ 这一波按 4× 压小，合并后的 chunk 数不变。
- ⚠️ **回读 + 合并路径被 ×4**（`VOLUMES = 4`）：`report_lod_requests` 每窗一次同步回读
  `LOD_REQ_WORDS × 4 = 8.4 MB`，随后是**逐字节**解包（210 万字 ≈ 30 ms 的标量循环）+ 两张 4 M 字
  稠密表的 `fill`（32 MB）+ 扫满 4 M 格建 `top`（≈ 11 ms）+ 每条一次 `% REQ_CAP`（100 万次除法 ≈ 17 ms）
  ⇒ 估计每 2 s 一次 60–70 ms 的尖峰（改动前是 1 M 字 / 15–25 ms）。**修**：映射区间按 `&[u32]` 整块
  memcpy 解包、只清"上一窗碰过的格子"、只遍历碰过的 key 建 `top`、环下标改增量退位 ⇒ 估计降到几 ms。
- 待办：**每级的空遍历**（射线逃出近场后要走满 L1/L2/L3 的窗口）与**帧率的最终定性**要靠
  `--features profile` 的逐 pass `PROF[...]`（本轮只有 `info` 级的 `REQ` / `UPLOAD` 可用）。

**第二轮实跑（`GATE_BENCH=orbit` + `--features profile`，本机复现"动一下就掉帧"）—— 根因找到并修掉：**

现象：进世界后 60 fps，一动相机就掉到 **17–23 fps**（用户机到个位数），而 GPU 逐 pass 只有 **7–9 ms**
（`gate_gi` 4.6 ms + `gate_dda_trace` 1.2–2.2 ms）；`stream_chunks` 3–13 ms、`prepare` 0.13 ms。
用临时计时（`DIAG[...]`，已删）定位到**全部时间在 `extract`：107–227 ms/帧**，并直接看到：

```
UPLOAD[full]: bytes=577.27MB (s 589067KB,…) chunks=2212 elapsed=124.19ms
UPLOAD[full]: bytes=578.52MB …             elapsed=200.65ms
```

**根因**：`VolumesBuilder::snapshot` 的 `bases_shifted` 以"各 volume 的 `b_struct` 长度"为判据。
远场级排在**主世界之前**（布局序 = 远场/物体在前、主世界最后，§8 的既定序），而远场一开始就往
`b_struct` 追加树块 ⇒ 主世界的 `tree_base` 每几帧漂移一次 ⇒ 降级**全量快照**：把四个 volume 拼一遍
（远场～0 + 主世界 577 MB）⇒ 每几帧一次 580 MB 的 memcpy + 上传。远场装得越多，触发越密
（`bases_shifted` 只要**长度**变；远场每加一块都变）。

**修**（§8 那条"物体变长的代价用**增长余量**摊薄"，落成远场级的固定预留区）：

- `brickmap/consts.rs::FAR_TREE_RESERVE_WORDS` = `FAR_POOL_CHUNKS(2048) × FAR_RESERVE_WORDS_PER_CHUNK(2048)`
  = **16 MB/级**（三级 48 MB）：远场 volume 一开场就把树区占到这个长度，块级分配从预留区切 ⇒
  `b_struct` 长度**恒定** ⇒ 布局不漂移。
- `BrickMapBuilder::{new_unbuilt_reserved, build_full_reserved}` + `VolumesBuilder::reserve_of`（只有远场级带预留）。
- `compact()` 对带预留的 volume **直接跳过**（它只会把长度压小；且预留区一开场就是空闲的 ⇒
  "空闲过半"判据恒真 ⇒ 每帧白搬一遍块）。
- `pool_capacity_chunks_far` 加**上限 `FAR_POOL_CHUNKS`**：容量超过预留就会真变长 —— 两者必须一起改。
- 回归测试 `far_reserve_keeps_tree_region_length_stable`：装 12 块 + 压实，`b_struct.len()` 全程不变。

**改后实测**（同一 `GATE_BENCH=orbit`、同一机位、`--features profile`）：

| | 改前 | 改后 |
|---|---|---|
| fps（60 s 内每 2 s 一个读数） | 17–23（进世界瞬态） | **57.5–60.5（全程）** |
| `UPLOAD[full]` | 每几帧一次 ×577 MB | **1 次**（启动 63.7 MB） |
| `extract` 尖峰 | 107–227 ms × 连续多帧 | 9 次（8 次集中在启动 4 s 内），1 次孤立 109 ms |
| 远场装载量（v1/v2/v3） | 10 / 35 / 30（被全量重传饿死） | **205 / 541 / 367**（cap 2048） |
| GPU 逐 pass | 7.4–11 ms | 7.0–11 ms（不变） |

残留：全程有 1 次孤立的 109 ms `extract`（无对应的全量重传 / 压实 / 窗口平移日志，未定性；
它落在 60 fps 的桶里，未造成可见掉帧）。要查的话用同一套 `DIAG[...]` 计时再跑一次。

**待实跑验收**（本轮只到"编译 + 单测通过"）：

1. 启动日志应出现三行 `FAR L{1,2,3} scale {4,16,64} … 覆盖 ±{0.66,2.62,10.49}km`。
2. `GATE_LOG=gate_render=debug,gate_app=debug` 下：`REQ[...]` 应出现 `v1/v2/v3` 的条目（早先只有
   `v0`）；`STREAM[v1 far gen… unload…]` 三行；`RESID[… | 远场 install … evict …]` 收敛（灌满后
   两者都回 0）。
3. 走廊方向对着 5 km 看：远场应是**稀疏的白格点阵**（格 1.28 m/5.12 m/20.5 m 各级），不是白墙、
   也不是空的。**用「世界」页的「暂停流式」把环冻住**，再飞出去看边界，确认远场确实铺到了 5 km。
4. 内存：三级远场常驻 ≤ 3 × 16384 块 × ≈ 20 KB ≈ 1 GB；与主世界的 2 GB 池加起来应显著低于进程
   上限（本机 16 GB）。`RESID` 行的"远场 install/evict"是核对点。
5. 帧率：远场把每条主射线的行程从 ~200 m 拉到 5 km（`t_cap` 也提到 21 km）⇒ **空走步数是新增的
   固定成本**。若掉帧明显，先看 `--features profile` 的 `gate_dda_trace`，再考虑加"每级的最小像素
   判据"（一级只在它的内沿处 ≥ 1 px 时才参与，否则跳过整个 grid）。
...
### 动态测试（第三轮：MC 地图 + 静态世界的空块洪水，2026-09-26）

方法：`--features profile` 拿**逐 pass GPU 均值**，`GATE_LOG=info,gate=debug,gate_app::debug_menu=debug`
拿 `RENDER 帧周期`（唯一"帧率限制在哪一侧"的量具）+ `RESID/STREAM/UPLOAD`；`video/vsync=false` 解开 60 帧帽
（否则看不出余量）；`GATE_BENCH=static`（相机不动）与 `=orbit`（相机 500 v/s 平移 + 转头）各跑一轮。
场景 = `mc_map`（Greenfield 城市，常驻 ~1766 块 / 141 MB）与 `nuke.vox`（82 块 / 145 MB）。RTX 3070、1280×720。

**帧时间落在哪一侧（MC 地图，静态，vsync off）**

```text
RENDER 帧周期 6.1 ms（163 fps）：渲染世界自身 0.4 ms，等别处 5.7 ms      ← "等别处" = 等主世界/等 present
GPU 逐 pass 均值（共 5.51 ms/frame）：
  gate_gi 2.49 + gate_dda_trace 1.68 = 76%；dda_face 0.54 / face_accum 0.15 /
  gi_flatten 0.11 / gi_temporal 0.10 / atrous×4 0.20 / godray+blur 0.11 / 其余 <0.05
STREAM 流式装载 0.02 ms/帧、RESID 常驻调度 0.06–0.40 ms/帧               ← CPU 侧两个大头都可忽略
```

⇒ **GPU 占帧周期的 87%，是限帧的一侧**；主线程那 5.7 ms 是等 present（pipelined rendering 里主世界跑在前面）。
CPU 侧（流式装载 + 常驻调度）合计 < 0.5 ms ⇒ 这两条路径没有优化空间可言（**先量再改**的价值就在这：
上一轮的"9 fps"真凶在装载路径，这一轮同样的量具证明它不是）。

**这一轮改掉的（都有实测对照）**

| 项 | 前 | 后 | 说明 |
|---|---|---|---|
| `nuke.vox`（静态 .vox）的请求洪水 | 2 831 521 条/窗、超容丢失 1 782 945、最热一块 157 707 票 | **0 条** | 静态世界没有生产源 ⇒ 渲染窗口内"没有内容"就是**确定是空**，直接写哨兵（`plan_residency` ①″）。补标 318/400 槽 |
| 已知空块哨兵同步 | 每次触发**全量重扫** 2000 块（爬坡期 O(n²) ≈ 200 万次哈希探测） | 只处理新增（`VolumeGrid::empty_log` 游标） | 顺带把 `empty_seq` 换成 `empty_count` 游标 |
| `TODO(diag)` 临时计时（两处） | 树区/显存扩容的耗时 | 删计时、留读数 | 实测：CPU `Vec::reserve` < 1 ms、GPU 缓冲扩容 100→134 MB = 1.1–1.4 ms ⇒ **不是**启动期卡顿来源 |

**新增的量具**：`gate_render::profiler::SysTimer`（诊断）：在系统体开头建一个，任何出口（含提前 `return`）
自动记账，每 60 次落一行 `<标签> X ms/帧`（`debug!`）。`RENDER 帧周期` 只回答"是不是 CPU 卡"，它回答
"卡在哪个系统"；本轮的 `STREAM 0.02 / RESID 0.3` 就是它给出的（⇒ 把嫌疑从 CPU 侧整个摘掉）。

**已知的一次性代价（不是瓶颈，但会看到）**

- 静态世界的**首次全量上传**：`UPLOAD[full]: 145.61MB … elapsed=53.5ms`（nuke，82 块）——
  一帧 53 ms ≈ 3 帧停顿。参照 M8 记录的 `infinite_cubes` 577 MB / 81–200 ms。
- 树区首拨：`infinite_cubes` 的一次 `Vec::reserve` 到 ~100 MB；GPU `b_struct` 扩容 100 → 134 MB（1.4 ms）。
- MC 地图**爬坡期**（前 ~2 s）：帧率 10 → 48 → 98 → 143 → 161（管道编译 + 上传 + 首批产出 + 哨兵补标）。
- `orbit`（500 v/s = 10 m/s，远超手动飞行 131 v/s）下帧率从 224 缓降到 ~121：装载跟不上"视线不断进入
  新区域"，请求量涨 27 倍（26K → 700K/窗，未丢）。手动速度下不出现。
...
### 10.10 转视角加载慢：每帧一报 + 视锥候选集 + 远场不饿死近场（2026-09-26）

**症状**：相机一转，远处半天不出来；量到"每秒只补 1–2 个 chunk"。

**三条根因，各自独立**：

1. **请求每 2 s 才换一批、且只报"最近 2 个缺失块"** ⇒ 帧额再大也没料可装。
   **修**：`profiler::ReqReadback` 把回读改成**每帧一趟的异步单缓冲流水**（`poll(Poll)` 非阻塞、
   读上一趟立刻备下一趟，数据最多落后一帧）。`REQ[...]` 日志仍按 `REPORT_PERIOD_SECS` 落（验收口径
   的节奏），行尾加 `读回 x/y 趟` 以核对流水没卡。
2. **半径补块的候选集合是"世界轴对齐的 ±`coarse_radius` 盒子"** ⇒ 任何方向都只够到
   `coarse_radius × 5.12 m`（缺省 8 ⇒ 41 m）；转视角后新露出的 41–164 m 全在盒外，只能靠请求一点点补。
   **修**（`infinite_cubes::plan_generation`）：候选集合改成**相机取向的视锥盒** —— 近处 `load_radius`
   的小盒（脚下/身后保底）+ 沿视线纵深到窗口边的锥（侧向 ±`coarse_radius`）+ 身后一条
   （`view_basis` / `CONE_ALONG_CHUNKS`）。同样的预算"沿视线伸出去"，够得到窗口边。
   单测 `view_cone_outranks_distance`（同距离时视野内必须排身后之前；`fill` 要够大以装下整个候选集，
   否则截断先吃掉身后那半，测的就不是排序了）。
3. **远场开工时把全局在飞槽位占满** ⇒ 近场饿死。MC 的远场块要读几百个 chunk 列（`mc::summary`），
   与近场块差三个数量级，而 `ChunkProducer` 的去重 / 在飞上限是**全局**的。
   **修**：`VolState::inflight` 逐卷计数 + `FAR_INFLIGHT_MAX`（= 2×worker 数）给每个远场级封顶。
   同一处再加**轮转起点**（`let first = frames % n_vol`）：`DISPATCH_PER_FRAME` / `READY_MAX` /
   `mount_count` 三个预算都是跨卷共用的，顺序遍历会让末尾那个卷永远捡剩的（实测 MC 的 L3 恒 0 常驻）。

**逐卷空块哨兵**（`plan_residency` ①′）：原先只做主世界（注释写"远场级没有空块来源"）—— 那是
`infinite_cubes` 的前提。MC 的远场级会大量产出 `None`（没建筑的格），而它的窗口铺满 ±10.5 km ⇒
不标哨兵时"空"与"没加载"的混淆会把请求环整圈打满。改成 `empty_cursor: Local<Vec<usize>>` 逐卷一游标
（换世界后日志变短 ⇒ 游标回退）。

**诊断**：`RESID` 行加"远场 N 块"（1..N 卷的 GPU 常驻合计）—— 没有它只能看到"远场 install 0"而误判
远场没上 GPU。

### 10.11 「静止也不收敛」：palette 认领新槽被当成"世界变了"（2026-09-26）

**症状**：MC 地图里 GI 噪声始终不收敛；静止有噪点，一动更明显，暗处靠自发光体素照明的面最差。

**取证**：`gate_render=debug` 数 `GI 二次顶点缓存 epoch → N`（它的触发源就是 `flags.z` 那个门的
`occluder_rev`）—— 静止时**每 ~0.5 s 自增一次**（`epoch 27 → 28`，相隔 0.5 s）。`flags.z = 0` 的那一帧
整屏不复用任何历史 ⇒ reservoir 的 `M` 每 0.5 s 从头来 ⇒ 永远到不了记忆窗，正是代码里早已记录的
"噪声严重、静态也不收敛"（`screen.wesl` 文件头 ⑥）。

**根因**：`plan_residency` 判「外观变了吗」用的是 **palette 写版本** —— 而 MC 的流式源每认领一个
**新槽**（`Pool::intern` 按方块状态去重）都会 `set` ⇒ 版本自增。新槽不影响任何已上传体素的外观，
却把 `wide_rev++` → `flags.z = 0` → 整屏丢历史。地图越大、相机动得越快，认领越频繁 ⇒ 越差。

**修法**（把两个问题分开）：
- `gate_voxel::Palette` 加 **`content_version`**：只在**覆写一个已占用的槽、且值真的变了**时自增
  （`set` 里比一次 `entries[i] != entry`）；`version` 继续只管"要不要重传这张表"。
- `brickmap::upload` 的 `dirty_aabb.palette_changed` 改用 `content_version`
  （`BuilderMirror::palette_content_versions` 逐卷记上次同步值）；`palette_dirty`（触发上传）不变。

**实测**（MC 地图，`GATE_BENCH=orbit`，40 s）：`epoch` 自增 **2 次（都在启动的 0.13 s 内）→ 之后 8 s 内 0 次**
（修前约 2 次/秒）；同一段里 palette 仍在认领新槽（13 次带 `pal>` 的上传）却不再作废历史。
副作用是好的：`gi_sec_slots` 二次顶点缓存也不再每 0.5 s 整表失效。

单测：`gate_voxel::palette::tests::content_version_ignores_slot_claims`。

### 10.12 试过并**已回退**：加宽 GI 面键位域（16 → 20 bit/轴）（2026-09-26）

**动机**：`gi/common.wesl` 的 `GI_KEY_LIMIT` 是**整条复用链的总闸门**（时域复用、空间复用、逐面认领、
`dda_main` 的逐面缓存着色全看 `gi_key_ok`），而它只有 16 bit/轴 = **±655 m**；MC 的 Greenfield 跨
±5.6 km、M8 的 L3 跨 ±10.5 km ⇒ 那些世界里"城里绝大多数位置整条链是关的"（表现：噪声永不收敛、
一动更明显、暗处靠自发光照明的面最差）。这与该常量的 CONSTRAINT 直接冲突，且它已经因为同一个原因
从 13 bit（±82 m）抬到过 16 bit。

**试过的改法**：word0 = x 20 + y 20、word1 = z 20 + face 3 + obj 9（±10.49 km），三个解码点
（`gi_den_same_plane`、`main.wesl` 的"从面键重建命中"）同步改。编译与 naga 校验都过。

**结果：不可用，已回退。** 实测出现两个回归 —— **玻璃变不透明** + 稳定噪点（此前是闪动噪点）。
两者都指向"键与控制流/材质取值的耦合比预期深"：键同时是 `face_slots` / `gi_sec_slots` 的槽哈希输入、
是导引有效位、是 `dda_main` 逐面缓存着色的查表键，位域一变就同时影响了这四件事。
**教训**：这个常量不是"位够不够"的问题，改它等于改 GI 的四条子系统的接口 —— 不能再靠加宽字段。

**下一步（未做）**：走**相机相对坐标**（键存相对相机的偏移体素），字段宽度不变、语义不变，
只是把"绝对坐标"换成"相对坐标"。那才是该 CONSTRAINT 的正解。

### 10.13 已落地（**曾误判为"已回退"，见文末更正**）：σ 不再单独决定历史权重（2026-09-26）

**动机**：`gi/denoise.wesl` ② 的 `w_var = clamp(σ_α/σ_use, 0.05, 1)` 在重尾估计（暗处被小发光体
照亮的像素）里恒塌到下限 0.05 ⇒ 时域累积等于被关掉。理由是"σ 回答'输入有多不可信'、不回答'变没变'，
而判'变没变'是 ⑤ 的 AABB 钳制"。改成"历史落在本帧邻域内就恒取 1"。

**结果：不可用，已回退。** 实测症状从"闪动噪点"变成**静止时暗处低频明暗涨落**（多个小片体素，
小面尤其明显），而且**运动时反而干净**。这一条恰好证伪了改动的前提、并给出一个重要的推论：
**本场景每帧的输入本身是干净的**（同面键邻域先平均过 + 离群钳制），把输出从"跟当帧"改成
"跟长历史（w → 31/32）"反而把长历史里的慢漂移暴露到画面上。⇒ σ 这条必须保留"跟着噪声水平走"。

**留下的**：同轮里那个 `Palette::content_version`（见 §10.11）是**独立且已被取证**的 ——
它把"每 0.5 s 整屏丢历史"消掉，正好解释了"运动时现在干净"（历史不再随相机移动被反复作废）。

**未做、下一步**：主 pass 的**逐面屏幕空间 GI 累加器**（`main.wesl` 的 `dda_face_accum` /
`dda_face_main`：把该面各 texel 的**降噪后** GI 定点累加进 `FACE_W_GI`，计数封顶
`GI_ACC_MAX_N = 1024`，再以 `gi_avg = sum/(n·4096)` **整面着色**）。它与"小片体素 + 低频涨落 +
大面稳定"三条现象逐条吻合，要先读清"累加器在什么条件下被清零 / 被别的面顶掉（槽是哈希来的）"。

### 10.14 已确认并修复：「重分配」在静止时把射线砍到 1/16~1/32（2026-09-26）

**症状**：静止时暗处（靠自发光体素照明的面最典型）有噪声涨落，**而且比运动时更脏**。最后半句是
唯一的硬约束——它排除了一大批猜测（估计方差、空间核、σ 权重本身都不是主因：那些只会让运动更差）。

**算术**（按实测机位的档位：降噪质量 高 ⇒ `cand_n0 = 8`、分帧 2、重分配 强 ⇒ `hi/lo = 4.0/0.25`）：
`cand_n = max(1, floor(cand_n0 × b_err / share + dither))`

| 情形 | `b_err` | 每 GI texel 每帧射线 |
|---|---|---|
| 静止（历史接上、`hist.m ≥ warm_m = m_cap_base × cand_n0`） | `lo` = 0.25 | `floor(8 × 0.25 / 2)` = **1** |
| 运动（历史接不上 ⇒ `cand_n0` 翻倍 = 16） | `hi` = 4.0 | `floor(16 × 4 / 2)` = **32** |

⇒ **32 : 1**。运动时噪声自然 ÷5.7 —— "静止还不如运动"不需要别的解释。

**根因是两个机制对同一份历史给出相反结论**：`重分配` 的前提是"窗填满 ⇒ 累积量已承载信息 ⇒
边际收益低、可以省射线"；而降噪侧的 `w_var = clamp(σ_α/σ_use, 0.05, 1)` 在**同一批像素**上独立判定
"这份历史不可信"（重尾估计 ⇒ σ_use ≫ σ_α）。于是那些像素退化成"**1 条射线 + 不被信任的历史**"。

**修法**（`gi/screen.wesl` 的 ②）：删掉低于基准的 `lo`，`b_err = select(hi, 1.0, 已填满)` ——
"重分配"回到字面语义：只把预算**重新分配**给没填满的像素，绝不把基准砍掉。档位仍控制 `hi`
（1.4/2.0/4.0），"填窗加速"一字未改。**稳态代价 = 原来的「重分配=关」那一档**（不新增开销）。

**实测**：把「重分配」拨到「关」（等价于恒 `b_err = 1.0`）⇒ 用户反馈"好了不少"，假设成立。

**本轮试过并已回退的**（都只留注释/WARNING，代码与 HEAD 逐位一致）：
1. 加宽 GI 面键位域 16 → 20 bit/轴（§10.12）：引入"玻璃不透明 + 稳定噪点"，回退；
2. 让 σ 不再缩历史权重（§10.13）：把输出从"跟当帧"改成"跟长历史"，出现低频明暗涨落，回退；
3. 让 atrous 的 `stab` 跟着"有效历史权重"而不是 `M`：无改善，回退。

**保留的**：`Palette::content_version`（§10.11，有取证）与逐面累加器的到顶折叠
（`dda_face_main`：`GI_ACC_MAX_N` 只封顶不遗忘曾是"面的值 = 头 1024 个样本的平均、此后冻结"）。

**仍剩的残噪**：`w_var` 把时域盒关掉（权重 0.05）后，只剩基准射线数在顶。要根治得让历史被真正用起来；
**下一次必须先搭 GPU 读回**（每个 GI 像素的 `M`、`w_var`、`hist_ok`/`pk_ok` 通过率，每秒一行），
不要再从静态代码推机制——本轮前三次失败都是这么来的。

**读回的落点（已探明，下一次是机械劳动）**：
- **不新增 binding**：`gi_guide` 是**一个 buffer 被两个 pass 共用**（`dda.rs` 分配一次，binding 6 与 10
  都绑它），且 GI pass 里 `gi_guide_out` 本就是 `read_write`、带 `COPY_SRC` ⇒ 计数器放在
  `px*GI_DEN_GUIDE_WORDS` 之后的尾部即可；`AuxTexCache` 是 render-world 资源（`gi.rs::prepare_gi` 就拿它），
  profiler 也能拿。只增不减 ⇒ 读回侧取**两次读数之差**算每帧均值，shader 侧不需要清零。
- **WARNING：`profiler::ReqReadback` 不能直接复用** —— 它的 `words` 是按 LOD 请求环缓冲的字数硬编码的，
  拿来读 8 个字会长度不匹配。要么给它加一个按字节数分配 `words` 的构造，要么另写一个 ~30 行的同款结构。
- **别把计数器留在没人读的状态**：`gi_ss_main` 是每 GI texel 每帧都跑的（1/2 档 + 2K ≈ 52 万 texel），
  7 个 `atomicAdd` 是实打实的开销 —— 本轮就是这么加进去又撤掉的。

**⚠️ 更正（2026-09-26 稍后）**：上面那段"不可用、已回退"的结论**是错的**。修掉同一轮里另两个独立
缺陷之后重试同一条改动 —— ①逐面累加器只封顶不遗忘、②`重分配` 静止时把射线砍到 1/16~1/32（§10.14）
—— 用户实测："**静态暗处噪声明显下降、低频涨落减少非常多**"。
**教训：一个改动失败时，先确认基线里没有别的已知缺陷在同时制造同一现象，再给它定罪。**
`cnt ≤ 1.5`（面只占一个 GI texel、邻域统计量拿不到）那一支原先是"退回按 σ 缩"，已补**逐像素相对
钳制** `clamp(hcol, c_in/K − pad, c_in·K + pad)` 顶替 ⇒ 判据对全部像素统一。
