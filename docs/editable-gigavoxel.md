...
| M2 | 叶代表色 **〔收窄，见 §9〕** | 给叶一个更好的代表色（现为"首个非空体素色"），用于压掉 `fp ≥ 1` 下 4 像素的块内替换误差；复用节点字高 16 位空位（§9） | 远景颜色正确性 | 编辑额外约 200–300 ns | 不碰 wire 尺寸；中，独立提交 |
...
| M4 | ray-guided 请求通道 | shader 有界 request buffer（chunk key + 所需档位 + 溢出计数）；Rust 异步回读（1–2 帧延迟）+ 合并排序 + 主射线优先于阴影 / GI；关闭时回退启发式半径 | 细节按真实需求分配 | 编辑走优先通道 | shader 与 Rust 各一处；中高 |
...
| 风险 | 缓解 | 处理时机 |
|---|---|---|
| M2 代表色的字段语义（16 位里"色 + 权重"怎么分） | 优先复用节点字高 16 位空位（§9 修正）；确实要改 wire 尺寸才走独立提交 + `DUMP_LAYOUT_VERSION` +1 + 一次全量重建 | M2 |
...
上层三档（`depth_cap <= 2`）要对着色射线启用，前提是 M2 给分裂节点一个代表色。

### M2 的一处事实修正（降低风险）

wire 的**节点字高 16 位**（`pack_node_palette` 的 bit16..31）原本就是「LOD 子树多数色」字段，现已整段
移除且恒 0（见 `chunk_tree.rs::pack_node_palette` 的 CONSTRAINT 注释），shader 侧所有节点读取都 `& 0xFFFF`。
⇒ M2 的代表色可以复用这 16 位：**不必 +1 word、不必 `DUMP_LAYOUT_VERSION` +1**。代价从"改 wire 尺寸"
降为"定字段语义"（16 位里怎么分"色 + 权重"待定；旧实现用众数，因其逐节点递归计票占序列化 88% 而被删，
这正是选加权平均的理由）。
...
1. **`infinite_cubes` 的窗口修复**：菜单「游戏/世界」→ 选 `infinite_cubes` → 「重载世界」。预期 = 近处
   ±10m 结构完整、无空洞、无轴对齐断口；超出后整片干净结束。若仍有断口 ⇒ 在 `plan_residency` 里加
   一条"CPU 有 / GPU 无"的常驻差值告警（现在没有这条日志，静默跳过发生在 `install_chunk` 的窗口检查里）。
...
2. **M2 叶代表色**（不碰 wire：复用节点字高 16 位空位，见 §9），把 `fp ≥ 1` 的 4 像素块内替换压下去。
3. **M4 ray-guided 请求通道**：把"相机距离当需求"换成"渲染 pass 发回请求"（`infinite_cubes` 正是它的
   验收台）；同时它也是 M0 步数直方图的读回通道。
