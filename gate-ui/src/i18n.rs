...
mod tests {
  ...
  #[test]
  fn resolve_falls_back_to_key_without_translator() {
    let t = UiTranslator::default();
    assert_eq!(t.resolve("menu.video"), "menu.video");
    assert_eq!(t.version(), 0);
  }

  #[test]
  fn bump_invalidates_version() {
    let mut t = UiTranslator::new(|k| format!("[{k}]"));
  ...
  #[test]
  fn refresh_rewrites_text_on_version_bump() {
    let mut app = App::new();
    app.insert_resource(UiTranslator::new(|k| match k {
      "menu.video" => "视频".to_string(),
      "menu.render" => "渲染".to_string(),
      other => other.to_string(),
    }));
    app.add_systems(Update, i18n_refresh_system);
    let e = app
      .world_mut()
      .spawn((I18nKey::new("menu.video"), Text::new("menu.video")))
      .id();
    app.update();
    assert_eq!(app.world().get::<Text>(e).unwrap().0, "视频", "首帧即解析");

    // 切语言：解析器换掉 + bump → 文本跟着变
    app.world_mut().resource_mut::<UiTranslator>().set(|k| match k {
      "menu.video" => "Video".to_string(),
      other => other.to_string(),
    });
    app.update();
    assert_eq!(app.world().get::<Text>(e).unwrap().0, "Video");

    // 版本未变 → 不再改写（把 Text 改脏后应保持不动）
    app.world_mut().get_mut::<Text>(e).unwrap().0 = "手动".into();
    app.update();
    assert_eq!(app.world().get::<Text>(e).unwrap().0, "手动");
  }

  #[test]
  fn refresh_rewrites_tooltip_text() {
    let mut app = App::new();
    app.insert_resource(UiTranslator::new(|k| format!("T:{k}")));
    app.add_systems(Update, i18n_refresh_system);
    let e = app.world_mut().spawn((I18nKey::new("tip.a"), Tooltip::new("tip.a"))).id();
    app.update();
    assert_eq!(app.world().get::<Tooltip>(e).unwrap().text, "T:tip.a");
  }
}