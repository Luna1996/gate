...
pub fn text_input(
  ...
  let mut ec = parent.spawn((
    ...
    Node {
      min_width: px(56.0),
      height: px(m.font_size.md + m.spacing.sm),
...
mod tests {
  ...
  fn num() -> TextInputKind {
    TextInputKind::Number { min: 1.0, max: 16.0, step: 1.0, decimals: 0 }
  }

  #[test]
  fn number_normalize_and_format() {
    assert_eq!(num().normalize(0.4), 1.0);
    assert_eq!(num().normalize(99.0), 16.0);
    assert_eq!(num().normalize(3.4), 3.0);
    assert_eq!(num().normalize(3.6), 4.0);
    assert_eq!(num().format(3.0), "3");
    assert_eq!(num().parse(" 5 "), Some(5.0));
    assert_eq!(num().parse("abc"), None);
    assert_eq!(TextInputKind::Text.parse("5"), None);
  }

  #[test]
  fn text_input_spawn_structure() {
    ...
    app.world_mut().entity_mut(root).with_children(|p| {
      h = Some(text_input(&ctx, p, TextInputConfig { text: "8".into(), kind: num(), ..default() }));
    });
    let e = *h.expect("spawned");
    let w = app.world();
    assert_eq!(w.get::<TextInputValue>(e).unwrap().0, "8");
    assert_eq!(*w.get::<TextInputKind>(e).unwrap(), num());
    assert_eq!(w.get::<FocusPolicy>(e).unwrap(), &FocusPolicy::Block);
    assert!(w.get::<crate::capture::MouseIntercept>(e).is_some(), "input swallows mouse");
    let children = w.get::<Children>(e).unwrap();
    assert_eq!(children.len(), 1, "one display text child");
    assert!(w.get::<TextInputText>(children[0]).is_some());
  }

  #[test]
  fn click_focuses_then_typing_edits_and_enter_commits() {
    ...
    app.add_systems(Update, (text_input_pointer_system, text_input_keyboard_system));
    let changes = Arc::new(Mutex::new(Vec::<String>::new()));
    let sink = changes.clone();
    app.add_observer(move |ev: On<TextInputChanged>| sink.lock().unwrap().push(ev.text.clone()));
    ...
    app.update();
    assert!(app.world().get::<TextInputState>(e).unwrap().editing);
    assert_eq!(app.world().resource::<TextInputFocus>().0, Some(e));
    // 裸 App 无 InputPlugin 清帧，手动清掉按下/释放的边沿
    app.world_mut().resource_mut::<ButtonInput<MouseButton>>().clear();

    // 输入 "3" → 值 123（超出 max，提交时归一为 16）
    ...
    app.update();
    assert_eq!(app.world().get::<TextInputValue>(e).unwrap().0, "123");
    ...
    app.update();
    assert_eq!(app.world().get::<TextInputValue>(e).unwrap().0, "16");
    assert!(!app.world().get::<TextInputState>(e).unwrap().editing);
    assert_eq!(app.world().resource::<TextInputFocus>().0, None);
    assert_eq!(*changes.lock().unwrap(), vec!["16".to_string()], "commit emits changed");
  }

  #[test]
  fn number_drag_changes_value() {
    use std::sync::{Arc, Mutex};
    let mut app = App::new();
    app.init_resource::<TextInputFocus>();
    app.init_resource::<ButtonInput<MouseButton>>();
    app.init_resource::<AccumulatedMouseMotion>();
    app.add_systems(Update, text_input_pointer_system);
    let changes = Arc::new(Mutex::new(Vec::<String>::new()));
    let sink = changes.clone();
    app.add_observer(move |ev: On<TextInputChanged>| sink.lock().unwrap().push(ev.text.clone()));
    ...
    app.world_mut().resource_mut::<ButtonInput<MouseButton>>().press(MouseButton::Left);
    app.update();
    // 向右拖 12px = 3 个 step（每 step 4px）→ 8 + 3 = 11
    app.world_mut().insert_resource(AccumulatedMouseMotion { delta: Vec2::new(12.0, 0.0) });
    app.update();
    assert_eq!(app.world().get::<TextInputValue>(e).unwrap().0, "11");
    assert!(app.world().get::<TextInputState>(e).unwrap().dragging);
    assert_eq!(*changes.lock().unwrap(), vec!["11".to_string()]);
  }
}
